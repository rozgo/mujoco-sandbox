...
def test_smoothness_changes_reward_without_filtering_physics():
    for body in (PRESETS['healthy'], next(b for b in LOSS_BODIES if b.name == 'whole_fr')):
        envs = [DogEnv(1, bodies=[body], randomize=False, faults=False,
                       threads=1, damage_action_rate_weight=w,
                       damage_angular_rate_weight=w) for w in (0, .1)]
        try:
            for step in range(20):
                action = np.full((1, 12), .2 * (-1)**step)
...
def test_nonexistent_joints_do_not_receive_added_smoothness_cost():
    ...
    envs = [DogEnv(1, bodies=[body], randomize=False, faults=False,
                   threads=1, damage_action_rate_weight=w) for w in (0, 10)]
    try:
        for step in range(20):
