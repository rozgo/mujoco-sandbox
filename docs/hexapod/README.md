# Sixlegs: dual-arm hexapod transfer

[Back to the demo gallery](../../README.md)
...
The floating base moves through leg contact forces; the objects are held by finger contact. The nominal task takes about 113 simulated seconds.
...
uv run --locked sixlegs view --speed 2 --camera head

# Frozen scene inspection:
uv run --locked sixlegs view --static

# Headless complete task; writes trajectory and success report into outputs/:
uv run --locked sixlegs run

# Tests include the complete physical transfer and rover missions:
uv run --locked pytest -q

# Static camera PNGs and model properties:
uv run --locked sixlegs render
uv run --locked sixlegs inspect

# Record a new run (bundled FFmpeg, no separate installation needed):
uv run --locked sixlegs record --speed 2

# Render the entire task with all six views, at real time:
uv run --locked sixlegs record --trajectory outputs/transfer.npz --layout all --speed 1 --output previews/transfer_all_views.mp4

# Render the shorter overview without repeating simulation:
uv run --locked sixlegs record --trajectory outputs/transfer.npz --speed 2
```
...
- [Masses, joint and torque limits, controller and contact design](../DESIGN.md)
- [Validation results and limitations](../VALIDATION.md)
- [Machine-readable nominal success report](../TRANSFER_REPORT.json)
...
`uv.lock` pins the environment. Git LFS tracks meshes, images, videos and model checkpoints. Virtual environments, caches, generated build artifacts and `outputs/` are ignored. The shared repository is [rozgo/mujoco-sandbox](https://github.com/rozgo/mujoco-sandbox); Mac and GPU work is synchronized through Git commits. See the [multi-machine workflow](../DEVELOPMENT.md).

To restore the vendored assets at the pinned upstream revision, run `uv run --locked python scripts/fetch_assets.py`. Normal runs use the checked-in assets and need no asset download.
