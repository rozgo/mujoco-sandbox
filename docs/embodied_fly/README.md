...
Latest result: [PPO learning progress and measured costs](HOVER_PPO_PROGRESS.md)
...
climbs approximately 106 mm in ten seconds: sustained flight and reduced drift
are demonstrated; stationary hover remains unfinished. The same full MaleCNS
actor controls all 78 outputs. The productive PPO stages train its existing
wing readout while upstream parameters stay fixed. [Final run details](runs/velocity_hover_ppo_05/SUMMARY.md)
record the continuation, preserved optimizer/critic, limitations and timing.
...
can establish learned flight. The subsequent [wing-output-only pilot](runs/wing_output_01/SUMMARY.md) preserves ground stability but worsens resting-wing posture and still fails hover. Its complete failed review is retained; run02 remains the development parent. A [physical decoder search](runs/sweep_search_01/SUMMARY.md) found no accepted combined improvement. A subsequent [state-readout fit](runs/state_readout_01/SUMMARY.md) improves reference-history prediction but still fails autonomous hover. [Actual-state corrections](runs/state_readout_02/SUMMARY.md) and an optional [motor-cell readout](runs/motor_cell_readout_01/SUMMARY.md) improve fitting but still fail the complete physical tasks. Both remain diagnostics; retention02 remains the development parent. The latter adds a small feedforward decoder inside the same brain, with all original weights frozen.
